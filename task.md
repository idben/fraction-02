...
### 6. 答題機制 ✅
- [x] 新增題庫系統（根據 plan.md 的分母2~10切分方式）
- [x] 新增題目生成函式（隨機選擇分母和切分方式，動態調整矩形高度）
- [x] 開始時用 dialog 顯示題目（小朋友語氣：「請把 1/3 塗上顏色喔！」）
- [x] 矩形中央顯示大字分數（如 1/3）
- [x] 小朋友需要自己畫線切分 + 填色
- [x] 新增計分系統（右上角只顯示「10 分」，不顯示題數和總分）
- [x] 新增「確認答案」按鈕
- [x] 新增答案驗證邏輯

### 7. 回饋機制 ✅
- [x] 新增音效播放功能（correct.mp3、wrong.mp3、victory.mp3）
- [x] 答對：播放音效 + 立繪瞬間出現（happy.png）+ dialog 顯示鼓勵話
- [x] 答錯：播放音效 + 立繪瞬間出現（sad.png）+ dialog 顯示正確示範
- [x] 答錯示範：系統自動展示正確的切割與填色方式
- [x] 答錯後顯示「我知道了」按鈕，讓小朋友重試
- [x] 重試答對不給分
- [x] 全部答對：播放 victory 音效 + pass.png 立繪

### 8. 視覺優化 ✅
- [x] 矩形寬度 800，高度根據分母動態調整（奇數分母時使用可整除的數字）
- [x] 手機上剛好一個螢幕，不出現捲軸
- [x] 立繪圖片在 backdrop 左下角（RPG 風格），只在答對/答錯瞬間出現
- [x] 立繪固定在畫面左下方，原始圖片尺寸的一半，bottom: -80px
- [x] 立繪移至 dialog 內部（因 top layer 特性），使用 position: fixed 固定在視窗左下角
- [x] Dialog 改用原生置中（移除手動 position/top/left/transform 設定）
...
- ✅ Hover 效果（點位、線條、填色區域）
- ✅ RWD 響應式設計（桌面、平板、手機）
- ✅ Dialog 完美置中顯示（使用原生 dialog 置中行為）
